...
export const useDialogStore = create<DialogStore>()(
  immer((set, get) => ({
    ...
    trigger: args => {
      ...
      set(state => {
        const isExists = state.dialogs.findIndex(x => x.id === dialog.id) > -1;
        if (!isExists) state.dialogs.push(dialog);
      });
    },
    closeById: id =>
      set(state => {
...
export function useStackingContext() {
  const stackingContextRef = useRef(
    useStackingContextStore.getState().stackingContext.length
  );
  ...
  const decrease = () => {
    ...
    useStackingContextStore.setState(state => ({
      stackingContext: [
        ...state.stackingContext.filter(x => x !== stackingContext),
      ],
    }));
  };
